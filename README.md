...
The careers page (`/careers.html`) has a fully working application form that uploads CVs and emails them directly to `careers@worldofhospitality.com.qa`. It's powered by `apply.php` in the site root.

### What it does

- Receives form data via POST (multipart/form-data)
- Validates name, email, phone, role, and CV file
- Validates CV is PDF / DOC / DOCX / RTF and ≤ 5MB
- Sanitizes inputs against header injection
- Has a honeypot field to deflect basic bot spam
- Builds a multipart email with the CV attached
- Sends via PHP's `mail()` function (built into HostGator)
- Returns JSON success/error to the form's async handler

### To customize

Open `apply.php` and edit the top:

```php
$RECIPIENT     = 'careers@worldofhospitality.com.qa';   // where applications go
$FROM_ADDRESS  = 'noreply@worldofhospitality.com.qa';   // sender — must be on your domain
$MAX_FILE_SIZE = 5 * 1024 * 1024;                       // 5MB
$ALLOWED_TYPES = ['pdf', 'doc', 'docx', 'rtf'];
```

That's it. No config files, no database.

### After deploying

1. Make sure `apply.php` lives in the same folder as `index.html` (i.e., `public_html/`).
2. Set up the inbox `careers@worldofhospitality.com.qa` in your HostGator cPanel → Email Accounts (or change `$RECIPIENT` to whatever inbox you actually use).
3. Send yourself a test application from `careers.html`. If it doesn't arrive, check:
   - Spam folder
   - That your hosting plan has PHP `mail()` enabled (HostGator does by default)
   - That the `$FROM_ADDRESS` is on a domain you own (cheap shared hosts sometimes block other-domain senders)

### If `mail()` deliverability is shaky

Some shared hosts route `mail()` output through low-trust IPs that get caught by spam filters. If applications don't arrive, the cleanest upgrade is to:

1. Sign up at **Formspree** (formspree.io) or **Web3Forms** (web3forms.com) — both free for low volume
2. Change `<form action="apply.php"` in `careers.html` to point at their endpoint
3. Add their access key as a hidden input

Or use PHPMailer with SMTP — happy to wire that up when you need it.
