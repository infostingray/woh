...
(() => {
  ...
  /* ============================================================
     0c) COUNTRY CODES — populate phone-group selects
     ============================================================ */
  const COUNTRY_CODES = [
    { name: 'Qatar',                code: '+974', flag: '🇶🇦', pri: 1 },
    { name: 'United Arab Emirates', code: '+971', flag: '🇦🇪', pri: 2 },
    { name: 'Saudi Arabia',         code: '+966', flag: '🇸🇦', pri: 3 },
    { name: 'Kuwait',               code: '+965', flag: '🇰🇼', pri: 4 },
    { name: 'Bahrain',              code: '+973', flag: '🇧🇭', pri: 5 },
    { name: 'Oman',                 code: '+968', flag: '🇴🇲', pri: 6 },
    { name: 'Lebanon',              code: '+961', flag: '🇱🇧', pri: 7 },
    { name: 'Jordan',               code: '+962', flag: '🇯🇴', pri: 8 },
    { name: 'Egypt',                code: '+20',  flag: '🇪🇬', pri: 9 },
    { name: 'Turkey',               code: '+90',  flag: '🇹🇷', pri: 10 },
  ...
  const phoneCodeSelects = document.querySelectorAll('.phone-group__code');
  if (phoneCodeSelects.length) {
    const priority = COUNTRY_CODES.filter(c => c.pri).sort((a, b) => a.pri - b.pri);
    const rest     = COUNTRY_CODES.filter(c => !c.pri).sort((a, b) => a.name.localeCompare(b.name));
    phoneCodeSelects.forEach((select) => {
      select.innerHTML = '';
      const addOption = (c, selected) => {
      ...
      priority.forEach(c => addOption(c, c.code === '+974'));
      const sep = document.createElement('option');
      sep.disabled = true;
      sep.textContent = '──────────';
      select.appendChild(sep);
      rest.forEach(c => addOption(c, false));
    });
  }
