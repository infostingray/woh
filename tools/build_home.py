...
from build_houses import live_tag, wall_tag, SHELL_HEAD, FOOT, HOUSES, proj, DOHA, LOGO, MENU_LOGOS, MOTIF, VERSION, CB
...
H = [
 ...
 dict(slug="al-beiruti", name="Al Beiruti", logo=LOGO['al-beiruti'], cls="",
      video="videos/al-beiruti.mp4", poster="images/brands/al-beiruti-table.jpg", still="images/brands/al-beiruti-room.jpg",
      ...
      facts=[("Cuisine","Lebanese, Levantine"),("Seating","180 and terrace"),("Status","Just opened")], caps=("The spread","The room")),
 dict(slug="eleven-green", name="Eleven Green", logo=LOGO['eleven-green'], cls="",
      video="videos/eleven-green.mp4", poster="images/brands/eleven-green-1.jpg", still="images/brands/eleven-green-2.jpg",
      ...
      facts=[("Cuisine","Burger bistro"),("Signature","The Bull Burger"),("Status","Opening soon")], caps=("The Bull","The tray")),
...
def wall_panel(h):
    ...
    <div class="wall__foot"><span class="wall__meta">{wall_tag(h['slug'], h['status'])}</span></div>
  </a>'''
...
def reel_slide(h, i):
    media = f'<picture><source media="(max-width: 900px)" srcset="images/brands/{h["slug"]}-m.jpg"><img src="{h["still"]}" alt="" loading="lazy"></picture>'
...
ROAD = [
 dict(stop="Founded", big="2019", city="Doha", note="World of Hospitality opens its first house in Doha, and runs it, rather than licensing it."),
 dict(stop="Concepts", big="Five", city="Four cuisines", note="Turkish, Indian, Lebanese, a burger bistro and an all-day Mediterranean table. Each brought from its home city."),
 dict(stop="People", big="300+", city="On the floor and in the kitchen", note="Senior-led teams on every shift. Recruited for the brand, trained to its own standard."),
 dict(stop="Head office", big="Landmark Mall", city="Doha", note="Finance, licensing, supply and reporting run centrally, so each house can concentrate on service."),
 dict(stop="In development", big="Two", city="Opening 2026", note="Eleven Green and Brunch &amp; Cake are in fit-out now. Both open before the year ends.", future=True),
 dict(stop="By 2028", big="Ten", city="Houses", note="The next five concepts are in conversation. Each one has to belong in Doha before it is signed.", future=True),
 dict(stop="By 2030", big="Three", city="Gulf markets", note="Doha first, then two more Gulf capitals with the same operating model.", future=True),
]
def road_nodes():
    out = ""
    for i, n in enumerate(ROAD):
        ...
        out += f'<li class="{cls}" style="left:{left:.2f}%" data-i="{i}" data-big="{n["big"]}" data-city="{n["city"]}" data-note="{n["note"]}" data-pct="{left:.2f}"><span class="road__hit"><span class="road__stop">{n["stop"]}</span><span class="road__dot"></span></span></li>'
    return out
...
<section class="road" id="road" aria-label="The road so far and the road ahead">
  <div class="road__stage">
    ...
    <div class="road__head">
      ...
      <p class="road__note" id="roadNote">World of Hospitality opens its first house in Doha, and runs it, rather than licensing it.</p>
    </div>
    <div class="road__rail">
      <div class="road__line"><span class="road__fill" id="roadFill"></span><span class="road__cursor" id="roadCursor"></span></div>
